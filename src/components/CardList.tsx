...
export default function CardList({ taskList, onDelete }: any) {
    ...
    const generateCard = () => {
        if (taskList.length === 0) {
            return <h3>Empty Now...</h3>;
        }

        let components = [];
        for (
            let i = itemsPerPage * currentPage - itemsPerPage;
            i < Math.min(itemsPerPage * currentPage, taskList.length);
            i++
        ) {
            components.push(
                <Card key={i} data={taskList[i]} onDelete={onDelete} />
            );
        }
        ...
        return (
            <div className="flex flex-col items-center">
                <div className="join mt-10">
                    <div className="mt-2">
                        <button className="join-item btn" onClick={decrease}>
                            «
                        </button>
                        <button className="join-item btn">{currentPage}</button>
                        <button
                            className="join-item btn rounded-r"
                            onClick={() => increase(totalPages)}
                        >
                            »
                        </button>
                    </div>
